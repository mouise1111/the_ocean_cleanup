...
import { useGlobalState } from "../Minigame/globalstate"; // Adjust the path as needed
...
const TimerExpiredPopUp = ({ onRestart, onStop }) => {
  ...
  return (
    <div className="fixed z-50 flex flex-col items-center justify-center p-8 transform -translate-x-1/2  border-8 border-amber-300 bg-amber-100 rounded-3xl top-1/4 left-1/2 w-96">
      <p className="mb-1 flex flex-row px-2 py-2 joti-one text-4xl text-sky-800 border-sky-500">
        Time's up!
      </p>
      <p className="text-lg">Final Score: {endScore}</p>{" "}
      {/* Display EndScore */}
